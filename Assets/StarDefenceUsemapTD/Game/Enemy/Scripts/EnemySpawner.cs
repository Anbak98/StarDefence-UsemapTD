...
namespace STARTD.Game.Enemy
{
    public class EnemySpawner : MonoBehaviour
    {
        ...
        public Queue<Wave> queue = new Queue<Wave>();
        public List<EnemyBehaviour> enemys;
        ...
        public async void Spawn()
        {
            ...
            while (curWave.infos.Count > 0)
            {
                for (int i = curWave.infos.Count - 1; i >= 0; i--)
                {
                    ...
                    HealthBarManager.Singleton.CreateHealthBar(enemy.transform, Color.red);
                    enemys.Add(enemy.GetComponent<EnemyBehaviour>());
                    // spawnCount 감소
                    waveInfo.spawnCount--;
        ...
        public async void SpawnElite(int index)
        {
            // 다음 스폰 전 대기 (0.5초)
            await Task.Delay(500);
            Navigation spawnPortal = StageManager.Singleton.CurStage.portalNavigations[index];
            Vector3 spawnPos = StageManager.Singleton.TileMap.CellToWorld(
                new Vector3Int(spawnPortal.x + StageManager.Singleton.TileMap.cellBounds.xMin, spawnPortal.y + StageManager.Singleton.TileMap.cellBounds.yMin, 0)
            );
            // 타일의 정중앙으로 이동
            ...
            // Enemy 생성
            GameObject enemy = Instantiate(elitePrefab, spawnPos, Quaternion.identity);
            HealthBarManager.Singleton.CreateHealthBar(enemy.transform, Color.red);
            enemys.Add(enemy.GetComponent<EnemyBehaviour>());
